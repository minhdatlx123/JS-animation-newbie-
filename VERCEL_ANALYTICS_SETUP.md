...
### 2. Add Analytics Script to Your HTML

For plain HTML sites like this project, add the following script to your `index.html` file in the `<head>` section:
...
**💡 Note:** When using the HTML implementation, there is no need to install the `@vercel/analytics` package. However, there is no route support. The script will still track page views and visitor data.
